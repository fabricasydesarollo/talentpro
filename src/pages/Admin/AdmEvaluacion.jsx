  ...
  const createEvaluacion = async (data) => {
    try {
      if (openEval.modo == 'crear') {
        ...
        const res = await axios.put(`${URLBASE}/evaluaciones/gestionar`, data=info)
        toast.success(`${res.data.message}, Finalizado con éxito!`)
        reset({ nombre: "", year: "", estado: "" })
        setRefreshData(!refreshData)
              ...
              <div className="space-y-2">
                {evaluaciones.map(evaluacion => (
                  <div
                    ...
                    className={`p-3 rounded-lg border cursor-pointer transition-all ${currentEval.idEvaluacion === evaluacion.idEvaluacion
                    ...
                    <div className="flex justify-between items-center gap-2">
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          reset({ nombre: evaluacion.nombre, year: Number(evaluacion.year), estado: evaluacion.activa, fechaInicio: evaluacion.fechaInicio, fechaFin: evaluacion.fechaFin })
                        }}
                        className="flex items-center gap-1 bg-znaranja hover:bg-znaranja/90 text-white px-3 py-1 rounded text-sm transition-colors"
